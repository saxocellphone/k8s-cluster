...
usage() {
  cat <<EOF
Usage: $0 <hipfire|image|gaming|off|status>

Modes:
  hipfire  Run the HIPFire LLM backend and stop ComfyUI/gaming.
  image    Run ComfyUI and stop HIPFire/gaming.
...
show_status() {
  echo "AI deployments:"
  kubectl_ai get deploy hipfire comfyui
  echo
  echo "AI pods:"
  kubectl_ai get pods -o wide
  echo
  echo "AI services:"
  kubectl_ai get svc hipfire comfyui
  echo
